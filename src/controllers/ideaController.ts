...
export const postIdea = async (req: Request, res: Response) => {
    ...
    const idea = new Idea({
        ...
        category,
        createdAt,
        userId, // Include userId in the idea document
    ...
    try {
        ...
        const pitchDocuments = pitchArray.map((pitch: { step: string; count: number }) => ({
            ideaId: ideaId, // Use the generated ideaId
            step: pitch.step,
            count: pitch.count,
        }));
            ...
            if (bannerFiles && bannerFiles.length > 0) {
                ...
                try {
                    ...
                    const thumb = new Thumb({
                        ...
                        ideaId: ideaId, // Use the correct ideaId
            ...
            if (otherFiles) {
                for (const file of otherFiles) {
                    ...
                    const newFile = new Document({
                        ...
                        size: file.size,
                        ideaId: ideaId, // Link file to the idea
...
export const likeIdea = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (existingLike) {
            ...
            // await sendNotifi(IdeaUser, `Your Idea Got A Like`, notificationTitle);
            // await createNotification('Your Idea Got A Like', 'Like', notificationTitle, datameta);
            res.status(200).json({ message: 'Idea liked successfully', liked : true });
            console.log("true");
        }
    } catch (error) {
